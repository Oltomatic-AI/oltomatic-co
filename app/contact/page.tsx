...
export default function ContactPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/contact", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, lang }) });
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <div>
                    ...
                    <select name="interest" value={form.interest} onChange={handleChange} style={inputStyle}>
                      {(es ? ["Consulta general", "OLTO Voz", "OLTO Suite", "Proyecto a medida"] : ["General enquiry", "OLTO Voice", "OLTO Suite", "Bespoke project"]).map(o => <option key={o}>{o}</option>)}
                    </select>
                  </div>
                  <div>
